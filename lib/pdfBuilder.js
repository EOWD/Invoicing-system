...
function computeOrderTotals(order, productLookup, config) {
  ...
  for (const line of order.lines) {
    ...
    subtotal += lineTotal;
    lines.push({ title: displayTitle, sku: line.sku, gtin, brand, batch, qty, unitPrice, lineTotal });
  }
...
function addInvoicePage(doc, order, totals, config, invoiceNumber, font, page) {
  ...
  const invColWidths = [26, 40, 68, 58, 173, 38, 28, 28, 64]; // Pos, Marke, EAN, Charge/Batch, Produkt, Menge, Preis, USt%, Gesamt
  const invColX = [MARGIN, MARGIN + 26, MARGIN + 66, MARGIN + 134, MARGIN + 192, 401, 439, 467, 495, PAGE_WIDTH - MARGIN];
  const tableTop = y;
  ...
  drawCellText(page, font, 'Produkt / Product', invColX[4], rowYs[1], invColWidths[4], { size: 7 });
  drawCellText(page, font, 'Menge', invColX[5], rowYs[1], invColWidths[5], { size: 7 });
  drawCellText(page, font, 'Preis', invColX[6], rowYs[1], invColWidths[6], { size: 7 });
  drawCellText(page, font, 'USt%', invColX[7], rowYs[1], invColWidths[7], { size: 7 });
  drawCellText(page, font, 'Gesamt', invColX[8], rowYs[1], invColWidths[8], { size: 7, align: 'right' });
  ...
  if (hasNoItems) {
    ...
    drawCellText(page, font, notAvailableText, invColX[4], rowY, invColWidths[4], { size: 7 });
    drawCellText(page, font, '0.00 ' + currency, invColX[8], rowY, invColWidths[8], { size: 7, align: 'right' });
  } else {
    for (let i = 0; i < totals.lines.length; i++) {
      ...
      const desc = (l.title || '').length > 48 ? (l.title || '').slice(0, 45) + '...' : (l.title || '');
      drawCellText(page, font, desc, invColX[4], rowY, invColWidths[4], { size: 7 });
      drawCellText(page, font, String(l.qty), invColX[5], rowY, invColWidths[5], { size: 7 });
      drawCellText(page, font, l.unitPrice.toFixed(2), invColX[6], rowY, invColWidths[6], { size: 7 });
      drawCellText(page, font, String(vatPct) + '%', invColX[7], rowY, invColWidths[7], { size: 7 });
      drawCellText(page, font, l.lineTotal.toFixed(2) + ' ' + currency, invColX[8], rowY, invColWidths[8], { size: 7, align: 'right' });
    }
  }
