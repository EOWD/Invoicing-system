...
/**
 * Parse semicolon-delimited SKU guide CSV (id;gtin;title or id;gtin;title;patch;price).
 * Returns array of { id, gtin, title, batch?, price? }.
 */
function parseSkuGuide(csvPath) {
  ...
  for (const line of rows) {
    ...
    if (id) result.push({ id, gtin, title, batch: batch || undefined, price: price != null ? price : undefined });
  }
  return result;
}
